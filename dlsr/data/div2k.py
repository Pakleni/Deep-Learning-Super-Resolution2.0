...
def crop(lr_img, hr_img, hr_crop_size=96, scale=2):
    ...
    lr_w = lr_img_shape[1] - lr_crop_size + 1
    lr_h = 0
...
class DIV2K:
    ...
    def dataset(
        self, batch_size=16, repeat_count=None, random_transform=True, crop_images=False
    ):
        ds = tf.data.Dataset.zip((self.lr_dataset(), self.hr_dataset()))
        if random_transform:
            ds = ds.map(
                lambda lr, hr: random_crop(lr, hr, scale=2), num_parallel_calls=AUTOTUNE
            )
            ds = ds.map(random_rotate, num_parallel_calls=AUTOTUNE)
            ds = ds.map(random_flip, num_parallel_calls=AUTOTUNE)
        elif crop_images:
            ds = ds.map(
                lambda lr, hr: crop(lr, hr, scale=2), num_parallel_calls=AUTOTUNE
            )
